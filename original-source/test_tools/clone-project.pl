...
my $usage = "./clone-project.pl parent-package-name orig-app-name app-path clone-path";

die $usage unless @ARGV == 4;

my ($parentPackage, $origAppName, $appPath, $clonePath) = @ARGV;

my $appName = $origAppName;
...
$codeDirLsOutput = `ls $codeDir`;
@codeFiles = split(/\s+/, $codeDirLsOutput);
my $importAwkCmd = "{sub(\"import $parentPackage.$oldPackageName\", \"import $parentPackage.$newPackageName\"); print}";
for my $codeFile (@codeFiles) {
    system("mv $codeDir/$codeFile $codeDir/$codeFile.old");
    system("awk '$importAwkCmd' $codeDir/$codeFile.old > $codeDir/$codeFile");
}
