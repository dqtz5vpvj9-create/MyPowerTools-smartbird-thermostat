...
def main():
    ...
    logger.info(As(f"date {datetime_class.now().strftime('%m%d%H%M%Y.%S')}", AsOption.STDOUT_NO_PRINT))
    As("mkdir -p /dev/memcg/test_art/")
    As(f"echo {args.mem}M > /dev/memcg/test_art/memory.limit_in_bytes")
    As("chmod 777 /dev/memcg/test_art/tasks")
    ...
    # No longer need to wait for art.pid, as the android runtime sets up memcg for itself
    # for i in range(args.round):
    #     time = datetime_class.now()
    #     prompt = f"Waiting for {i+1}/{args.round} art.pid"
    #     logger.info(prompt)
    ...
    #             raise e
    #     logger.info(f"Found art.pid in round {i+1}/{args.round}: {pid}")
    # if args.freq:
    #     while True:
    #         keep_freq()
    #         sleep(0.1)
