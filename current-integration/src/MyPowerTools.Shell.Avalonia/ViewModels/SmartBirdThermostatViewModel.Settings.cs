...
public sealed partial class SmartBirdThermostatViewModel
{
    ...
    {
        ShowConsoleCommand = new AsyncRelayCommand(() =>
        {
        ...
        TestConnectionsCommand = new AsyncRelayCommand(TestConnectionsAsync, () => !IsBusy);
        StartEnergyServerCommand = new AsyncRelayCommand(StartEnergyServerAsync, () => !IsBusy);
        StopEnergyServerCommand = new AsyncRelayCommand(StopEnergyServerAsync, () => !IsBusy);
    }
    ...
    {
        if (SaveSettingsCommand is AsyncRelayCommand save) save.NotifyCanExecuteChanged();
        if (TestConnectionsCommand is AsyncRelayCommand test) test.NotifyCanExecuteChanged();
        if (StartEnergyServerCommand is AsyncRelayCommand start) start.NotifyCanExecuteChanged();
        if (StopEnergyServerCommand is AsyncRelayCommand stop) stop.NotifyCanExecuteChanged();
    }
