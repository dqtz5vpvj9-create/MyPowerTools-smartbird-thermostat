...
public sealed class WebToolBridgeRequestEventArgs(string requestJson) : EventArgs
{
...
public sealed class SmartBirdWebView : Control
{
    ...
    public static readonly StyledProperty<string> ToolIdProperty =
        AvaloniaProperty.Register<SmartBirdWebView, string>(nameof(ToolId), "smartbird");
    ...
    private int _currentState = (int)SmartBirdWebViewState.Loading;
    ...
    public event EventHandler<WebToolBridgeRequestEventArgs>? BridgeRequest;
    ...
    public Task SendBridgeResponseAsync(string responseJson)
    {
        var payload = JsonNode.Parse(responseJson) ?? throw new InvalidDataException("Bridge response is empty.");
        return SendCommandAsync(new { type = "bridge-response", payload }, _generation);
    }
    ...
    private void StartHost()
    {
        ...
        startInfo.ArgumentList.Add(string.IsNullOrWhiteSpace(ToolId) ? "web-tool" : ToolId);
        ...
        try
        {
            ...
            _ = WatchHostLoadingAsync(process, generation, cancellation.Token);
            UpdateHostBounds();
        }
        catch (Exception ex)
        {
    ...
    private async Task WatchHostLoadingAsync(Process process, int generation, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (generation == _generation &&
                ...
                !process.HasExited &&
                Volatile.Read(ref _currentState) == (int)SmartBirdWebViewState.Loading)
            {
                Dispatcher.UIThread.Post(() =>
                {
                    if (generation == _generation && ReferenceEquals(process, _hostProcess))
                    {
                        ...
                        NotifyState(
                            SmartBirdWebViewState.Failed,
                            "Web tool loading timed out. Check the configured URL or open it in the system browser.");
                        StopHost();
                    }
                });
            }
        ...
        HostProcessEvent hostEvent)
    {
        ...
        if (hostEvent.Kind == HostProcessEventKind.BridgeRequest)
        {
            BridgeRequest?.Invoke(this, new WebToolBridgeRequestEventArgs(hostEvent.Value));
            return;
        }
        ...
        out HostProcessEvent hostEvent)
    {
        ...
        try
        {
            ...
            if (string.Equals(type, "bridgeRequest", StringComparison.Ordinal) &&
                root.TryGetProperty("payload", out var bridgePayload))
            {
                var requestJson = bridgePayload.GetRawText();
                if (requestJson.Length is > 0 and <= MaximumHostFrameLength)
                {
                    hostEvent = new HostProcessEvent(
                        HostProcessEventKind.BridgeRequest,
                        SmartBirdWebViewState.Ready,
                        "",
                        requestJson);
                    return true;
